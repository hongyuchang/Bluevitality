...
普通模式：
	...
	dd              	删当前行，3dd删除三行
	x   			删除光标后面1个字符
	i、a			i进入单词前编辑，a进入单词后编辑、I进入行首编辑、A进入行尾编辑、- 跳转到行首
    	o、O            		在当前行下行编辑、在当前行前一行编辑
	u、.	  		撤销、重复上次操作 .与ctl+r类似	
	cc			删当前行并进入编辑模式
	^、$			至行首、尾，- 到上一行的行首
	d^  d$			从光标到行首、行尾删除
	...
	[[			跳转到代码块的开头去，要求代码块中'{'必须是单独占的一行
	ZZ              	不进入命令模式执行wq或x而直接退出
	{、}            		代码块的起使和结束
	$               	移至行尾
	gg              	至文档开始，5gg 跳到第五行，G:到文档尾部
	s               	删除光标后1个字符并进入插入模式，类似cc，S 删除光标所在行并进入插入模式
	*               	跳转到当前光标所处单词的下一次出现
	>G              	增加与当前行相同缩进的行的缩进层级（从左向右）
	f1              	搜索并跳转到当前行出现的第一个1字符
	20.             	重复20次上次的修改
	==              	自动与前一行的缩进对齐!
	3==             	后三行与前一行保持缩进
	<c-[>           	同ESC
...
命令模式：
	...
	:Sex			水平分割当前窗口，并在一个窗口中开启文件浏览器,按下<Enter>可以打开
    	...
    	:数字                 	定位文件的第几行
    	:!ls /etc           	执行本地命令
    	:s /A/B/g           	把当前光标所处的行中的A替换成B，并把所有A高亮显示；
    	:%s /A/B            	把文档中所有A替换成B
    	:#,# s /A/B/g       	＃号表示数字，表示从多少行到多少行，把A替换成B
...
vim +#              要打开的文件名 （其中#是打开文件时光标定位的位置行号)
vim +/内容            要打开的文件名 (/内容是打开文件时光标定位到这个内容位置)
vim a.txt b.txt c.txt (可以打开多个文件，输出n是切换下一文件，N是上一个文件)
vim +'set nu' ../config/elasticsearch.yml   显示行号
