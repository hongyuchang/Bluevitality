...
[root@localhost ~]# cp /usr/share/openldap-servers/DB_CONFIG.example /var/lib/ldap/DB_CONFIG    #拷贝数据库配置文件？
[root@localhost ~]# chown -R ldap. /var/lib/ldap/
[root@localhost ~]# systemctl enable slapd
[root@localhost ~]# systemctl start slapd      #默认使用389端口（不使用认证的情况下）
...
olcSuffix: dc=my-domain,dc=com                 #所在的域/域后缀
olcRootDN: cn=Manager,dc=my-domain,dc=com      #管理员帐号的dn（此处名为Manager）
...
#  -x 
#  -H 连接
#  -D 指定使用的身份？...官方说明：bind DN
#  -b 官方说明：base dn for search
