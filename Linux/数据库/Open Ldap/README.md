#### 备忘 ( 轻量量目录访问协议：LDAP )
```txt
目录是一个为查询浏览、搜索而优化的专业分布式数据库，它在逻辑上呈树状结构组织数据，就象Linux/Unix中的文件目录一样
...
     LDAP为人员组织机构中常见的对象都设计了属性，如：commonName，surname。属性的冒号后面必须留空格，值的后面不能有空格
     属性支持高级的过滤功能
     
     c：  国家
     l：  地名，如城市或者其他地理区域的名字
...
数据交换格式:  LDIF
     "LDAP Data Interchange Format" 是LDAP数据库信息的一种文本格式，用于数据导入/出，每行都是"属性:值"对
