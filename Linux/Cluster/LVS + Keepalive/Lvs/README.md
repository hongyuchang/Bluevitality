...
```txt
DR：
      DR、RS均使用相同VIP对外服务但仅DR对此VIP的ARP请求进行响应，所有RS对此VIP的ARP请求保持静默，即网关把对VIP的请求全部交给DR
      当DR收到后根据调度算法找出对应RS并切把此VIP的数据包中的MAC改为RS的MAC（因IP一致）并将请求分发给这台RS
      RS处理后由于IP一致因此可将数据直接返回给C端，"相当于直接从客户端收到这个数据包无异"
      因DR要对二层包头改换所以DR、RS之间必须在同一个广播域（可简单理解为在同一台交换机中）
      
      特点：
      与TUN相比这种实现不需隧道结构因此可使用大多数操作系统做为后端节点，但其要求负载均衡器的网卡必须与物理网卡在一个物理段
      RS的网关不能与DR的网关相同，接收VIP的网关应该仅知道DR的MAC地址...

NAT：
      将C端来的数据包目的IP换为某台RS的IP并发其处理，RS处理后把数据交给LVS(网关)后LVS再把RS发送时数据包中的源IP改为VIP后发送
      不论进出流量都必须经过负载均衡，"性能不优"（NAT模式下的RS需将网关指向DR，数据包内的客户端IP一直存在数据包中，未被DR改变）
